module Avatax
  module Api
    ...
    class Customers < Base
      ...
      ##
      # Link bill-to customer to a list of ship-to customers
      # @see https://developer.avalara.com/api-reference/avatax/rest/v2/methods/Customers/LinkShipToCustomersToBillCustomer/
      #
      # @param company_code [String] The company_code in avatax.
      # @param customer_code [String] The customer_code in avatax.
      # @param args [Hash] Arguments for avatax.
      # @return [Avatax::Response]
      #
      def link_ship_to(company_code, customer_code, args = {})
        raise ArgumentError, 'company_code is required' if company_code.blank?
        raise ArgumentError, 'customer_code is required' if customer_code.blank?

        resp = connection.post "api/v2/companies/#{company_code}/customers/billto/#{customer_code}/shipto/link", args
        handle_response(resp)
      end
    end
  end
